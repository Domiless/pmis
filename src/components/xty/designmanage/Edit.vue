        ...
        <a-form :form="form">
          ...
          <a-row>
            <a-form-item label="需求数量" :labelCol="{span:3}" :wrapperCol="{span:18}">
              <a-input v-decorator="['number',{rules: [{validator: chickNumber}]}]"></a-input>
            </a-form-item>
          </a-row>
          <a-row>
  ...
  created() {
    ...
    setTimeout(() => {
      this.form.setFieldsValue({
        workOrderId: this.oneMsg.workOrderId,
        designNo: this.oneMsg.bomNo,
        projectName: this.oneMsg.projectName,
        partName: this.oneMsg.partName,
        bomDrawingNo: this.oneMsg.bomDrawingNo,
        number: this.oneMsg.number,
        remake: this.oneMsg.remake
      });
    }, 100);
  },
  watch: {
